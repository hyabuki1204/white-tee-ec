...
| **月次予算** | **¥20,000/月**。想定運用の約2倍で、通常は当たらない。暴走を止める最後の壁として置く（第 12.4 節） |

**Phase 0 は完了済み。Phase 1 から着手できる。**
...
**Midjourney に一般公開 API は存在しない（2026-08 時点、一次情報で確認）。**

- Midjourney 公式の updates ページに **Enterprise API の「調査中」アンケート**があり、
  リリース済みの製品ではない。申し込み・意見受付の段階
- 利用規約は**自動化を明確に禁止**しており、例外は個別に明示的に許諾された場合のみ
- 市場にある「Midjourney API」は**すべて Discord / Web を自動化するサードパーティ
  ラッパー**。つまり規約違反の上に成り立っている

**結論: production 用途では Midjourney 系プロキシを採用しない。**
公式 API を持つプロバイダが十分な品質に達しているため、規約違反リスクを
負う理由がない（第 6.1.1 節）。

以下のリスク表は、それでもサードパーティ経由を選ぶ場合に何を引き受けることに
なるかの記録として残す。
...
### 6.1.1 実際に採用するプロバイダ候補（2026-08 調査）

すべて**公式 API** を持ち、規約違反の論点が存在しない。

| プロバイダ | 単価（1枚・1MP 級） | 強み | 本件での位置づけ |
|---|---|---|---|
| **FLUX 2 Pro**（Black Forest Labs） | 約 **$0.03** | **参照画像を 9〜10 枚同時に使えて、スタイル一貫性が現状最強**。生地・肌の質感、浅い被写界深度の写真的表現に強い | **第 1 候補** |
| **Imagen 4**（Google） | Fast $0.02 / Standard $0.04 / **Ultra $0.06** | 写実性が最も高い。肌・生地・光・反射の再現が実写と見分けにくい | **第 2 候補**（フェイルオーバー兼 A/B 比較用） |
| **GPT Image 2**（OpenAI） | 約 $0.03（1K）/ $0.05（2K）/ $0.06（4K） | 複雑で条件の多いプロンプトの遵守が最も正確。文字描画も強い | 予備 |
| **Adobe Firefly Image Model 5** | 要確認 | **ライセンス済み／パブリックドメインのみで学習**した唯一の選択肢。IP 面が最も安全 | 権利面を重視する方針に変えた場合の選択肢 |

**なぜ FLUX 2 Pro を第 1 候補にするか。**

ブランディング画像で効くのは、1 枚の完成度よりも**シリーズとしての一貫性**である。
Instagram のフィードを縦に見たとき「同じ人が同じカメラで撮っている」と感じられることが
ブランド認知を作る。1 枚ずつ最高でも毎回テイストが変わるなら、ブランドとしては弱い。

FLUX 2 Pro の**マルチ参照画像**はまさにこの用途で、確立した見え方を参照として渡すと
それを保持したまま新しい絵を作れる。本設計の `GenerationRequest.referenceImageUrls` と
`ProviderCapabilities.supportsReferenceImage` はこの機能に対応済み。

> **追加設計が必要な点**: 一貫性を効かせるには、参照画像をブリーフごとではなく
> **ブランド共通の「参照セット」として永続化**し、production のジョブに常に添付する
> 仕組みが要る。現設計にはこの概念がない。Phase 6 で
> `image_reference_sets` テーブルを追加する想定（第 11 章 Phase 6 参照）。
...
`image_cost_ledger` の当月合計が `IMAGE_MONTHLY_BUDGET_JPY`（**既定 ¥20,000**）を超えたら、
...
- [x] **月次予算の上限額** → **¥20,000/月**（第 12.4 節）
- [x] 環境変数の名前を確定（第 12.1 節）

**完了条件**: 達成済み。**Phase 0 は完了。Phase 1 に着手できる**
...
- [x] vision で生成画像を評価、alt text / キャプション案を生成
      （`lib/images/director/qa.ts`）
- [x] tick に組み込み（`lib/images/jobs/qa-runner.ts`）
- [x] レビュー画面に verdict と issues を表示（Phase 5 で実装済み）
- [x] **QA 失敗時にジョブを落とさない** — 例外を握りつぶして評価なしでキューへ
- [x] Claude のコストを `image_cost_ledger` に記録

**設計からの変更点**

1. **QA 用の画像はダウンサイズする前提にした**（`QA_MAX_IMAGE_BYTES`）。
   フル解像度は1枚あたり最大 約4,784 入力トークンで、4枚だとパイプラインの
   Claude コスト全体を支配する。構図・白飛び・破綻の判定にその解像度は要らない。
2. **`white_reproduction` を独立した評価軸にした。**
   白Tシャツのブランドなので、白がグレーや青に転ぶ／階調が飛ぶのは
   「細かい指摘」ではなく実質的な欠陥。汎用の品質スコアに埋めない。
3. **alt text は QA が下書きし、人間が編集して承認する。**
   承認時に日本語 alt text 必須のチェックは残してあるので、
   AI の下書きがそのまま公開されることはない。

**完了条件**: 達成（実 API 呼び出しを除く）。
verdict は**並び順とハイライトにのみ使い、自動承認には使わない**。
...
| `IMAGE_MONTHLY_BUDGET_JPY` | 月次上限（第 12.4 節） | `20000` |
...
5. ~~月次予算の上限額~~ → **¥20,000/月**（第 12.4 節）

**Phase 0 のブロッカーはすべて解消済み。以下は後続フェーズで決めればよい。**

6. **管理者が複数人か**（Phase 5 まで） — 承認の監査ログを機能させるには個人識別が要る
   （現状は共有パスワード）。エージェント承認を入れるぶん、`actor` の区別は今より重要になる
7. **プロバイダの最終決定**（Phase 6 まで） — 調査は完了（第 6.1.1 節）。
   推奨は **第1候補 FLUX 2 Pro / 第2候補 Imagen 4**。
   Midjourney 系プロキシは規約違反リスクを負う理由がないため不採用。
   Phase 3〜5 はプロバイダ非依存なので、着手前に決める必要はない
...
**結論: 月次上限 ¥20,000。** 想定運用の約2倍で、通常運用では当たらない水準。

#### 単価の前提

| 項目 | 単価 | 確度 |
|---|---|---|
| Claude Opus 5 | $5 / 100万入力トークン、$25 / 100万出力 | 確認済み |
| Claude Sonnet 5 | $3 / $15（2026-08-31 まで導入価格 $2 / $10） | 確認済み |
| プロンプトキャッシュ | 書き込み 1.25倍、読み出し 0.1倍 | 確認済み |
| 画像1枚の入力トークン（高解像度） | 最大 約 4,784 トークン | 確認済み |
| **画像生成 API（1ジョブ = 4枚）** | **¥18**（FLUX 2 Pro $0.03/枚）〜**¥36**（Imagen 4 Ultra $0.06/枚） | 2026-08 の公開価格 |

為替は ¥150/USD 換算。第 6.1.1 節の候補で実際の公開価格を確認済み。
当初の目安 ¥10〜60 の範囲内に収まっており、**月次上限 ¥20,000 の妥当性は変わらない。**
契約時点で再確認すること（この分野は価格改定が多い）。

#### 1サイクルあたり

「ブリーフ1本 → コンセプト4案 → 1案承認 → 4枚生成 → 1枚採用」を1サイクルとする。

| 工程 | モデル | 概算 |
|---|---|---|
| Stage 1 コンセプト生成 | Opus 5 | ¥14 |
| Stage 2 RenderSpec | Sonnet 5 | ¥2 |
| Stage 3 QA（4枚 vision） | Sonnet 5 | ¥14 |
| **Claude 小計** | | **¥30** |
| 画像生成（1ジョブ4枚） | プロバイダ | ¥10〜60 |
| **合計** | | **¥40〜90** |

思考トークン込み（Opus 5 は思考がデフォルト ON）。
**Claude 側で最も重いのは Stage 3 の vision** — QA 用途に高解像度は不要なので、
リサイズすれば Stage 3 は半分以下になる。

#### 月次シナリオ

実際には1枚採用するのに **平均3サイクル**回る前提で見積もる（AI 画像は歩留まりが悪い）。

| シナリオ | 内容 | 月額 |
|---|---|---|
| **A. テスト期間**（Phase 1〜5） | mock のみ。画像生成コストゼロ | 〜¥600 |
| **B. 想定運用** | IG 週2本 = 月8本採用。24ジョブ / 96枚生成 | ¥1,000〜2,200 |
| **C. 積極運用** | IG 毎日 + EC 季節ビジュアル = 月40本採用。120ジョブ / 480枚生成 | ¥5,000〜11,000 |

Phase 1〜3 は Claude も使わないため実質ゼロ。**予算が未確定でも Phase 5 までは進められる。**

#### なぜ ¥20,000 か

積極運用の約2倍。この水準なら:

- 通常運用では**絶対に当たらない** → 警告が鳴ったら本当に異常
- 事故は止まる → リトライ制御のバグで 1,000 ジョブ暴走すれば ¥60,000。上限があれば ¥20,000 で止まる

リトライ上限・サーキットブレーカー・冪等キーを設計に入れてあるが、
**それらが全部効かないケースを想定した最後の壁**。
運用実績に合わせて絞り込むのではなく、余裕を持たせて
「鳴ったら異常」の状態を保つほうが警報として機能する。

閾値: 80%（¥16,000）で管理画面に警告、100% で新規ジョブ投入を停止（第 8.6 節）。

#### コストが跳ねる要因

| 要因 | 影響 | 対処 |
|---|---|---|
| 再指示ループ | `revision` 上限5まで回すと1本 ¥450 | 上限を3に下げる余地あり |
| 1ジョブの枚数 | 4枚→8枚で単純に倍 | `desired_variant_count` の既定を上げない |
| Stage 3 の画像解像度 | 高解像度のまま渡すと1枚 約4,784トークン | QA 前にリサイズ |
| Opus 5 の effort | `xhigh` にすると思考トークンが増える | コンセプト生成は `high` で十分 |

#### キャッシュの位置づけ

ブランドコンテキスト（約2,000トークン）はキャッシュ対象になる
（Opus 5 の最小キャッシュ長は 512 トークンなので条件を満たす）。

ただし **この規模ではコスト削減効果は月 ¥100 程度**で、コスト目的では意味がない。
**採用理由はレイテンシ**（コンセプト生成の待ち時間短縮＝管理画面の体感）であり、
コスト最適化の施策として数えないこと。
