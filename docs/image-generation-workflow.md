...
7. **プロバイダの第 1 候補と第 2 候補**（Phase 6 まで） — 2 系統目を最初から想定しておくかどうか。
   実単価が確定するのもここ（第 12.4 節の画像生成コストは目安のまま）
...
| **画像生成 API（1ジョブ = 4枚）** | **¥10〜60** | ⚠️ **目安。Phase 6 で要検証** |

為替は ¥150/USD 換算。画像生成 API の単価は MJ 系プロキシ / Replicate / OpenAI Images で
桁が違い、価格改定も多い。**Phase 6 でプロバイダを決めた時点で実測して置き換えること。**
