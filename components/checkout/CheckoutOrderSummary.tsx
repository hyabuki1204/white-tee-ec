    ...
    <div className="mx-auto mt-10 max-w-sm space-y-4 text-left">
      <ul className="space-y-3 border-t border-neutral-200/70 pt-6">
        {summary.lineItems.map((item, index) => (
          <li
            key={`${item.name}-${index}`}
            className="flex justify-between gap-4 text-[11px] font-light tracking-[0.04em] text-neutral-600"
          >
      ...
      <dl className="space-y-1.5 border-t border-neutral-200/70 pt-4 text-[11px] font-light tracking-[0.04em] text-neutral-500">
        {summary.shipping > 0 ? (
